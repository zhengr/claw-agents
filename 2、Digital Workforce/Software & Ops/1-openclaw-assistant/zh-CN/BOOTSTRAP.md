...
- 在 TOOLS.md 中写明：
  - 输入路径（如草稿、日志、拆解报告的读取位置）。
  - 输出路径（如草稿、发布日志、反馈的写入位置）。
  - 格式或字段约定（如草稿结构、日志列）。
...
- 与配置者（或既有管线文档）约定本智能体的「完成」含义（如「草稿已写入路径 X」「发布日志已更新」）。
- 若有下游智能体（发布、数据助手等），确保输出格式与字段与下游一致。
...
- 配置者可确认 `TOOLS.md` 中文档索引 URL（如 `https://docs.openclaw.ai/llms.txt`）在目标环境中可访问。
- 若有企业代理或内网 Gateway，仅在使用人主动提供时记录于 TOOLS.md「环境备注」；不在此处写密钥。
