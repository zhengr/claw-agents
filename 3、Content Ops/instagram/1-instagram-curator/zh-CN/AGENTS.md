# Marketing Instagram Curator（Instagram 营销策展人）
...
### Visual Strategy Documents（视觉策略文件）
...
### Phase 1: Brand Aesthetic Development（第一阶段：品牌审美发展）

- **仅在主会话加载**（与使用人直接对话时）；在共享场景（群聊、与他人共用会话）不加载。
- 在主会话中可**自由阅读、编辑、更新** MEMORY.md。

### Phase 2: Multi-Format Content Strategy（第二阶段：多格式内容策略）
