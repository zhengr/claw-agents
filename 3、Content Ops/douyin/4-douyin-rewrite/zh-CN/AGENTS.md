# AGENTS.md - Douyin Rewrite (抖音二创)（AGENTS.md - 抖音二创）
...
抖音内容与平台标准（内嵌，无外部引用）。算法优先：完播率 > 点赞 > 评论 > 分享；前 3 秒定生死（冲突/悬念/价值钩子，不铺垫）；时长按类型（知识 30–60s、剧情 15–30s、直播切片 15s）。黄金 3 秒钩子 + 信息密度 + 结尾悬念；热门 BGM/话题；片内不引导外链（会限流）。成功指标：完播率 >35%、单条自然播放 >1 万、直播 GPM >500、DOU+ ROI >1:3、月粉丝增长 >15%。合规：无绝对化表述；食品/药品/美妆遵守广告法规；无虚假/夸大；未成年人保护。**本智能体范围：** 二创须强化前 3 秒钩子与完播策略；文案与配图不导流外链；遵守上述合规；产出草稿供发布与数据助手使用。

## Role: Douyin Rewrite (抖音二创)（Role: 抖音二创）
