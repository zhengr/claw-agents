...
## 预设技能（ClawHub 为主，两源取最优）

**ClawHub 技能一览**见 [CLAWHUB-SKILLS.md](./CLAWHUB-SKILLS.md)，**skills.sh 技能一览**见 [SKILLS-SH-SKILLS.md](./SKILLS-SH-SKILLS.md)。各智能体在 `config/openclaw-wechat-article-fragment.json` 中已绑定技能。合并后 OpenClaw 按 `agents.list[].skills` 加载；需先将技能安装到 OpenClaw 可加载的 skills 目录，**安装后目录名与 config 中 `skills` 数组一致**。

| Agent id                        | 默认技能列表                                                                 | 用途说明 |
|---------------------------------|------------------------------------------------------------------------------|----------|
| wechat-article-viral-breakdown  | wechat-article-extractor, baoyu-format-markdown                             | 文章提取（ClawHub/skills.sh）+ 拆解输出规范 |
| wechat-article-rewrite         | baoyu-cover-image, baoyu-article-illustrator                               | 封面与长文配图 |
| wechat-article-publisher       | wechat-ai-publisher, baoyu-markdown-to-html, baoyu-compress-image, baoyu-post-to-wechat | 发布（ClawHub 或 skills.sh）+ 排版与压缩 |
| wechat-article-data-assistant  | baoyu-format-markdown                                                      | 数据报告格式规范 |
| wechat-article-hot-monitor     | baoyu-url-to-markdown, baoyu-format-markdown                               | 抓取热点链接、规范日报格式 |
| wechat-article-write           | baoyu-cover-image, baoyu-article-illustrator                              | 原创内容配图与封面 |
| wechat-article-comment-manager | （按需从 ClawHub/skills.sh 选评论采集与回复技能）                             | 评论拉取、回复草稿、情感摘要 |
...
**来源：skills.sh（取最优）** — Baoyu 系列与公众号专项（wechat-article-extractor、wechat-article-publisher 等）见 [SKILLS-SH-SKILLS.md](./SKILLS-SH-SKILLS.md)。
