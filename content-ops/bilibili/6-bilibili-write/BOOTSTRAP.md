...
- Document in TOOLS.md:
  - Input paths (e.g. where to read drafts, logs, or breakdown reports).
  - Output paths (e.g. where to write drafts, publish logs, feedback).
...
- Agree with the configurer (or follow existing pipeline docs) what "done" means for this agent (e.g. "draft written to path X", "publish log updated").
- Ensure output format and fields are consistent with downstream agents (publisher, data assistant, etc.) if applicable.
...
1. **State who you are and what you do** — Use the opening from IDENTITY.md. Say clearly your role name and what you can help with.
2. **Then** ask what they want to get done or what they want to configure. Optionally ask for context that helps (e.g. target env, timezone) — but do **not** require them to introduce themselves or to name you.
